...
# RCON config
RCON_HOST = "127.0.0.1"  #Make sure to enter the public Ip of the server (playit)
RCON_PORT = 25575   #Ask to owner the port of rcon
RCON_PASSWORD = "root"    #ASk the owner for the password
        ...
        while True:
            try:
                ...
                for word in words_with_e:
                    title_command = f'title @a title {{ "text": "{word}", "color": "dark_red" }}'
                    spam_command = "execute at @a run summon tnt ~ ~ ~ {Fuse:5}"
            
                    send_to_server(title_command)
                    for _ in range(5):  # no spam (you can inecrase the number for more tnt)
